...
При `TAKE_PROFIT` пользователь не должен получать голый watchdog alert. Agent-driven cron обязан повторно прочитать fresh positions/open orders/quotes/fees/margin и **сначала проверить полную исполнимость закрытия обеих ног** на exact qty с учётом top-of-book size, tick/qty metadata и безопасной последовательности short-first. Если у любой ноги отсутствует нужный bid/ask, недостаточен доступный размер или полный двухногий close сейчас не может быть подготовлен как исполнимая immutable-карточка, cron обязан завершиться молча: не отправлять TAKE_PROFIT, оценку P&L, объяснение блокера или напоминание. Уведомление разрешено только когда обе ноги полностью исполнимы сейчас. Тогда сравнить закрытие с ожиданием до expiry, показать фиксируемый P&L, максимум при благоприятной expiry, упускаемую выгоду, комиссии и освобождаемую маржу, подготовить новый immutable single-use close plan/artifact и запросить отдельный `CONFIRM`; до него число trade POST равно нулю. Закрывать short PUT первой и только после terminal full fill — long PUT. Пустой stdout watchdog остаётся бесшумным.
